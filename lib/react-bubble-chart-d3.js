...
var BubbleChart = function (_Component) {
  ...
  _createClass(BubbleChart, [{
    ...
    value: function renderBubbles(width, nodes, color) {
      ...
      //Set font-size and style for node text
      ...
      //
      // node.selectAll(".label-text").text(function (d) {
      ...
      //               return line;
      //           } else {
      //               return "<tspan dy='1.2em'>" + line + "</tspan>";
      //           }
      //       }).join("");
      //   } else {
      //       return d.label;
      ...
      node.selectAll(".label-text").append('tspan')
      .text(function(d) {
      ...
      .attr('x', 0)
        ...
        var height = self.node().getBBox().height;
        var lines = d?.label?.split("\n");
        return lines.length > 1 ? -(height/2) : 3;
      })
      ...
      .append('tspan')
      .attr('dy', function(d) {
        return typeof d.data.fontSize !== "undefined" ? d.data.fontSize + 4 : labelFont.size + 4;
      })
      .text(function(d) {
        var lines = d.label.split("\n");
        return lines[1];
